...
class SimulationUtil():
    ...
    def load_user_click_data(self, user_id):
        ...
        print("User " + str(user_id) + ": Loaded " + str(symbol_click_df.shape[0]) + " calibration clicks, and " + str(text_click_df.shape[0]) + " clicks!")
        return full_click_df

    def load_user_phrase_data(self, user_id):
        # load the real phrases that the user typed
        phrase_data_fname = os.path.dirname(parentdir) + "/Nomon_User_Data/OSF Data/text_entry_task/user_"\
                            + user_id + "_text_phrase_data_clean.csv"
        text_phrase_df = pd.read_csv(phrase_data_fname, usecols=["Session Num", "Phrase Text"])
        return text_phrase_df

    def run_job(self, my_task_id, num_tasks):
        parameters_li = ["A", "B", "C", "D", "F", "G"]
        ...
        for job_num, job in enumerate(task_jobs):
            ...
            params = {"click_df": self.load_user_click_data(job),
                      "phrase_df": self.load_user_phrase_data(job)}
